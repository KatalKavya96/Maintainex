...
export function PinGrid({ pins, loading, error, onChanged, onDelete }: { pins: Pin[]; loading: boolean; error?: string; onChanged: () => void; onDelete: (pin: Pin) => void }) {
  if (loading) return <div className="rounded-xl border border-line bg-white p-8 text-sm text-slate-500 shadow-soft">Loading pins...</div>;
  ...
  if (!pins.length) return <div className="rounded-xl border border-line bg-white p-8 text-center text-sm text-slate-500 shadow-soft">No pins yet. Add your first contribution shortcut.</div>;
  return (
