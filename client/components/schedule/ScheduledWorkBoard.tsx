...
export function ScheduledWorkBoard({ items, loading, error, onChanged, onDelete }: { items: ScheduledWork[]; loading: boolean; error?: string; onChanged: () => void; onDelete: (item: ScheduledWork) => void }) {
  if (loading) return <div className="rounded-md border border-line bg-white p-8 text-sm text-slate-500 shadow-soft">Loading scheduled work...</div>;
  ...
  if (!items.length) return <div className="rounded-md border border-line bg-white p-8 text-center text-sm text-slate-500 shadow-soft">No scheduled work yet.</div>;
  return (
    <div className="grid gap-4 xl:grid-cols-3 2xl:grid-cols-6">
      {scheduledWorkStatuses.map((status) => {
        const columnItems = items.filter((item) => item.status === status);
        return (
          <section key={status} className="rounded-md border border-line bg-slate-50 p-3">
